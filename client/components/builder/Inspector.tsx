...
export default function Inspector({ nodes, setNodes, selectedId }: InspectorProps) {
  const selected = nodes.find((n) => n.id === selectedId) ||
    nodes.flatMap((n) => (n.children || [])).find((n) => n.id === selectedId) || null;
      ...
      for (let i = 0; i < next.length; i++) {
        ...
        if (n.children) {
          ...
          if (cidx !== -1) {
            ...
            children[cidx] = { ...children[cidx], props: { ...children[cidx].props, ...patch } };
            next[i] = { ...n, children };
            return next;
          }
        }
      ...
      {selected.type === "section" && (
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => updateProps({ paddingY: v })} value={selected.props.paddingY}>
              <SelectTrigger id="paddingY"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => updateProps({ background: v })} value={selected.props.background}>
              <SelectTrigger id="background"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="bg-white">White</SelectItem>
                <SelectItem value="bg-secondary/60">Subtle</SelectItem>
                <SelectItem value="bg-gradient-to-br from-primary/10 to-fuchsia-200/40">Brand Gradient</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            <Label htmlFor="align">Align</Label>
            <Select onValueChange={(v) => updateProps({ align: v })} value={selected.props.align}>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => updateProps({ align: v })} value={selected.props.align ?? "left"}>
              <SelectTrigger id="alignH"><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      {selected.type === "paragraph" && (
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="textP" value={selected.props.text} onChange={(e) => updateProps({ text: e.target.value })} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="alignP">Align</Label>
            <Select onValueChange={(v) => updateProps({ align: v })} value={selected.props.align ?? "left"}>
              <SelectTrigger id="alignP"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="left">Left</SelectItem>
                <SelectItem value="center">Center</SelectItem>
                <SelectItem value="right">Right</SelectItem>
      ...
      {selected.type === "button" && (
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="label" value={selected.props.label} onChange={(e) => updateProps({ label: e.target.value })} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="href">Link URL</Label>
            <Input id="href" value={selected.props.href} onChange={(e) => updateProps({ href: e.target.value })} />
          </div>
        </div>
      )}
      ...
      {selected.type === "image" && (
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="src" value={selected.props.src} onChange={(e) => updateProps({ src: e.target.value })} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="alt">Alt text</Label>
            <Input id="alt" value={selected.props.alt} onChange={(e) => updateProps({ alt: e.target.value })} />
          </div>
        </div>
      )}
      ...
      {selected.type === "two-column" && (
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => updateProps({ gap: v })} value={selected.props.gap}>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <textarea
              ...
              value={(selected.props.items || []).map((it: any) => `${it.time} | ${it.title} | ${it.location}`).join("\n")}
              onChange={(e) => {
                const items = e.target.value
                  ...
                  .filter(Boolean)
                  .map((l) => {
                    const [time, title, location] = l.split("|").map((s) => s.trim());
      ...
      {selected.type === "speaker" && (
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="name" value={selected.props.name} onChange={(e) => updateProps({ name: e.target.value })} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="role">Role</Label>
            <Input id="role" value={selected.props.role} onChange={(e) => updateProps({ role: e.target.value })} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="photo">Photo URL</Label>
            <Input id="photo" value={selected.props.photo} onChange={(e) => updateProps({ photo: e.target.value })} />
          </div>
        </div>
      )}
