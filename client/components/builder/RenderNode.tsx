...
function widthStyleFor(node: BuilderNode) {
  ...
  if (w === "1/2") return "50%";
  if (w === "custom")
    return node.props?.customWidth ? `${node.props.customWidth}px` : "800px";
  return "100%";
}
...
export function RenderNode({
  ...
  const commonStyle: React.CSSProperties = {
    transform: node.props?.rotate
      ? `rotate(${node.props.rotate}deg)`
      : undefined,
  ...
  const Wrapper = ({ children }: { children: React.ReactNode }) => (
    <div
      ...
      className={`mx-auto ${wrapperClasses}`}
      style={{
        maxWidth: maxW,
        position: node.props?.absolute ? "relative" : undefined,
      }}
    >
      {children}
    </div>
    ...
    case "section": {
      ...
      return (
        <section className={`${background}`}>
          <div
            className={`container mx-auto ${paddingY} flex flex-col ${align} gap-4`}
          >
            {node.children?.map((c) => (
      ...
      return (
        <div className={`grid grid-cols-1 md:grid-cols-2 ${gap}`}>
          <div className="flex flex-col gap-4">
            {left && <RenderNode node={left} setNodes={setNodes} />}
          </div>
          <div className="flex flex-col gap-4">
            {right && <RenderNode node={right} setNodes={setNodes} />}
          </div>
        </div>
      );
    }
            ...
            <table className="w-full text-sm">
              <tbody>
                {items.map((it: any, i: number) => (
                  ...
                  >
                    <td className="px-4 py-3 font-medium text-foreground w-24 whitespace-nowrap">
                      {it.time}
                    </td>
                    <td className="px-4 py-3">{it.title}</td>
                    <td className="px-4 py-3 text-muted-foreground w-40 whitespace-nowrap">
                      {it.location}
                    </td>
                  </tr>
                ))}
